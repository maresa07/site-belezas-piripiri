...
def upload_to_firestore():
    ...
    for category in data["cat_locations"]:
        cat_id = category["id"]
        cat_ref = db.collection("cat_locations").document(cat_id)
        batch.set(cat_ref, {
            "nome": category["nome"],
    ...
    for i in range(0, total, batch_size):
        ...
        for location in batch_locations:
            ...
            category_ref = db.collection("cat_locations").document(location["category_ref"])
            user_ref = db.collection("users").document(location["user_ref"])
